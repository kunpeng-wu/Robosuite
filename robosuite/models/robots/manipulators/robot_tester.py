...
class RobotTester:
    ...
    def _load_controller(self):
        ...
        self.controller_config["robot_name"] = self.robot_model
        ...
        self.controller_config["joint_indexes"] = {
            ...
            "qpos": self.joint_pos_indexes,
            "qvel": self.joint_vel_indexes,
        }
        self.controller_config["actuator_range"] = self.torque_limits
        self.controller_config["policy_freq"] = 20
        self.controller_config["ndim"] = len(self.robot_model.joints)
    ...
    def _apply_gripper_action(self, action):
        ...
        """
        gripper_action_actual = self.gripper.format_action(action)
    ...
    def gripper_actuator_ids(self):
        return [self.sim.model.actuator_name2id(x) for x in self.gripper.actuators]

    @property
    def ee_pos(self):
        return np.array(self.sim.data.site_xpos[self.sim.model.site_name2id(self.eef_name)])

    @property
    def eef_name(self):
        return self.gripper.important_sites["grip_site"]
...
if __name__ == '__main__':
    ...
    robot = create_robot("Humanoid")
    # robot = create_robot("Panda")
    from robosuite.models.grippers import gripper_factory, NullGripper
    gripper = gripper_factory("HandLeft", 0)
    # gripper = gripper_factory("Robotiq85Gripper", 0)
    # gripper = NullGripper()
    tester = RobotTester(robot, "0 0 0.2", "1 0 0 0", gripper, 0, 0.1)
    ...
    # gripper
    print(tester.gripper.dof)
    print(tester.gripper.joints)
    print([tester.sim.model.joint_name2id(jnt) for jnt in tester.gripper.joints])
    print(tester.gripper.actuators)
    print(tester.gripper_actuator_ids)

    tester.viewer.set_camera(0)

    action = np.zeros(7)   # 6 arm + 5 hand
    # action = np.zeros(6)   # 6 arm
    # action[6:] = 1
    # action[3] = 0.4
    # for step in range(tester.step_time):
    #     if step > 200:
    #         action[3] = -0.4
    #     tester.step(action)
    #     tester.render()
