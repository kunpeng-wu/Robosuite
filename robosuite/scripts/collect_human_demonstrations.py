...
def collect_human_trajectory(env, device, arm, env_configuration):
    ...
    env.close()
    print("collect one trasition")


def collect_human_trajectory_bimanual(env, device, arm, env_configuration):
    """
    Use the device (keyboard or SpaceNav 3D mouse) to collect a demonstration.
    The rollout trajectory is saved to files in npz format.
    Modify the DataCollectionWrapper wrapper to add new fields or change data formats.
    ...
    while True:
        ...
        # Get the newest action
        action, grasp = input2action_Bimanual(
            device=device, robot=active_robot
        )
...
if __name__ == "__main__":
    ...
    tmp_directory = os.path.join("./tmp/", time_str)
    # tmp_directory = './tmp/20240109150222'
    env = DataCollectionWrapper(env, tmp_directory)

    # initialize device
    # if args.device == "keyboard":
    from robosuite.devices import Keyboard, KeyboardBimanual
    ...
    t_now = time.time()
    time_str = datetime.datetime.fromtimestamp(t_now).strftime('%Y%m%d%H%M%S')
    new_dir = os.path.join("./demonstrations/", time_str)
    os.makedirs(new_dir)
    ...
    # gather_demonstrations_as_hdf5(directory="./tmp/20240109150222", out_dir="./demonstrations/20240109150222", env_info=env_info)
    # env.close()