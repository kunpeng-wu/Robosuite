...
if __name__ == '__main__':
    ...
    arm = SingleArm("Humanoid", gripper_type="HandLeft", controller_config=controller_config)
    arm.load_model()

    xml = arm.robot_model.get_xml()
    from robosuite.utils.binding_utils import MjSim
    sim = MjSim.from_xml_string(xml)
    arm.reset_sim(sim)
    arm.setup_references()
    print(arm.robot_model.dof)
    print(arm.robot_model.actuators)
    print(arm._ref_joint_actuator_indexes)
    print(arm.robot_model.joints)
    print(arm.eef_rot_offset)
    print(arm.torque_limits)

    arm._load_controller()
