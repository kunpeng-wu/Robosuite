...
class Bimanual(Manipulator):
    ...
    def action_limits(self):
        ...
        low, high = [], []
        for arm in self.arms:
            # low_g, high_g = (([-1] * self.gripper[arm].dof, [1] * self.gripper[arm].dof) if self.has_gripper[arm] else ([], []))      # dof = 12
            low_g, high_g = (([-1] * self.gripper[arm].control_dim, [1] * self.gripper[arm].control_dim) if self.has_gripper[arm] else ([], []))  # action = 1
    ...
    def _action_split_idx(self):
        ...
        return (
            ...
            self.controller["right"].control_dim + self.gripper["right"].control_dim
