...
class MujocoEnv(metaclass=EnvMeta):
    ...
    def _get_observations(self, force_update=False):
        ...
        # {'robot0_proprio-state': [robot0_joint_pos_cos, robot0_joint_pos_sin, ...], 'object-state': [cube_pos, cube_quat, gripper_to_cube_pos]}
        ...
        for modality, obs in obs_by_modality.items():
            ...
            if modality == "image-state" and not macros.CONCATENATE_IMAGES:
                continue
            observations[modality] = np.concatenate(obs, axis=-1)   # {'robot0_joint_pos_cos': [], 'robot0_joint_pos_sin': [], ..., 'robot0_proprio-state': [], 'object-state': []}
        ...
        Returns:
            4-tuple:
                - (OrderedDict) observations from the environment
                    ...
                    - robot0_eef_pos 3
                    - robot0_eef_quat 4
                    - robot0_gripper_qpos 16
                    - robot0_gripper_qvel 16
                    - cube_pos 3
                    - cube_quat 4
                    - gripper_to_cube_pos 3
                    - robot0_proprio-state: [robot0_joint_pos_cos, ..., robot0_gripper_qvel] 57
                    - object-state: [cube_pos, cube_quat, gripper_to_cube_pos] 10
