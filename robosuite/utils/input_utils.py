...
def input2action(device, robot, active_arm="right", env_configuration=None):
    ...
    if controller.name == "IK_POSE":
        ...
        # dpos = dpos * 75 if isinstance(device, Keyboard) else dpos * 125    # 0.05 * 75 = 3.75
        dpos = dpos * 20 if isinstance(device, Keyboard) else dpos * 125    # 0.05 * 20 = 1.0
    ...
    dpos = np.clip(dpos, -1, 1) # clip dpos to [-1, 1]
...
def input2action_Bimanual(device, robot):
    """
    Converts an input from an active device into a valid action sequence that can be fed into an env.step() call
    ...
    for arm in ["right", "left"]:
        dpos[arm], rotation[arm], raw_drotation[arm], grasp[arm] = (
            state["dpos"][arm],
            state["rotation"][arm],
            state["raw_drotation"][arm],
            state["grasp"][arm]
        )
    ...
    gripper_dof = {}
    for arm in ["right", "left"]:
        controller[arm] = robot.controller[arm]
        gripper_dof[arm] = robot.gripper[arm].control_dim

    # First process the raw drotation
    drotation = {}
    action = {}
    action_process = np.array([])
    for arm in ["right", "left"]:
        drotation[arm] = raw_drotation[arm][[1, 0, 2]]
        # Flip z
        drotation[arm][2] = -drotation[arm][2]
        # Scale rotation for teleoperation (tuned for OSC) -- gains tuned for each device
        drotation[arm] = drotation[arm] * 1.5 if isinstance(device, KeyboardBimanual) else drotation[arm] * 50
        dpos[arm] = dpos[arm] * 75 if isinstance(device, KeyboardBimanual) else dpos[arm] * 125
    ...
    #     print("Error: Unsupported controller specified -- Robot must have either an IK or OSC-based controller!")
        ...
        # map 0 to -1 (open) and map 1 to 1 (closed)
        grasp[arm] = 1 if grasp[arm] else -1

        # Create action based on action space of individual robot
        if controller[arm].name == "OSC_POSITION":
            action[arm] = np.concatenate([dpos[arm], [grasp[arm]] * gripper_dof[arm]])
        else:
            action[arm] = np.concatenate([dpos[arm], drotation[arm], [grasp[arm]] * gripper_dof[arm]])

        # concatenate [right_arm right_hand left_arm left_hand] action
        action_process = np.concatenate([action_process, action[arm]])
    # Return the action and grasp
